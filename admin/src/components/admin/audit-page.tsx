...
export function AuditPage() {
  ...
  const columns = useMemo<ColumnsType<AuditEventDetail>>(
    () => [
      { title: 'Action', dataIndex: 'action', key: 'action', render: (value: string) => <Tag>{value}</Tag> },
      { title: 'Resource', dataIndex: 'resource_type', key: 'resource_type', render: (value: string) => value || '-' },
      { title: 'Resource ID', dataIndex: 'resource_id', key: 'resource_id', render: (value?: string) => value ? <Text code>{value}</Text> : '-' },
      { title: 'Reason', dataIndex: 'reason', key: 'reason', render: (value?: string) => value || '-' },
      { title: 'Created', dataIndex: 'created_at', key: 'created_at', render: (value?: string) => value ? new Date(value).toLocaleString() : '-' },
      ...
      <Drawer title="审计详情" open={detailOpen} onClose={() => setDetailOpen(false)} width={520}>
        {detail ? (
          <div className="space-y-4">
            <div><Text type="secondary">Action</Text><div>{detail.action}</div></div>
            <div><Text type="secondary">Before</Text><pre className="overflow-auto rounded bg-slate-100 p-3 text-xs">{detail.before || '-'}</pre></div>
            <div><Text type="secondary">After</Text><pre className="overflow-auto rounded bg-slate-100 p-3 text-xs">{detail.after || '-'}</pre></div>
            <div><Text type="secondary">Contract Gaps</Text><div>{detail.contract_gaps?.join(', ') || '-'}</div></div>
          </div>
        ) : null}
      </Drawer>
    </div>
