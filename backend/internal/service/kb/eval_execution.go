...
type EvalRunExecutor interface {
	Execute(ctx context.Context, run *model.KBEvalRun, dataset []evaluation.DatasetCase) (*evaluation.Report, error)
}

type DefaultEvalRunExecutor struct{}

func NewEvalRunExecutor() EvalRunExecutor {
	return &DefaultEvalRunExecutor{}
}

func (e *DefaultEvalRunExecutor) Execute(ctx context.Context, run *model.KBEvalRun, dataset []evaluation.DatasetCase) (*evaluation.Report, error) {
	...
	runner := &evaluation.Runner{
		Factory: func(profile evaluation.StrategyProfile) (evaluation.Searcher, error) {
			return buildSearcher(cfg, manager, profile, cfg.Milvus.CollectionName)
		},
	}
	return runner.Run(
		ctx,
		dataset,
		[]evaluation.StrategyProfile(run.Profiles),
		evaluation.GateThresholds(run.GateThresholds),
		run.BaselineProfile,
