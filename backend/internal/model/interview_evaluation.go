...
type (
	...
	InterviewEvaluation  struct {
		...
		UserID     uint                   `json:"user_id" gorm:"not null;index:idx_user_id;uniqueIndex:uniq_eval_user_report_deleted;comment:用户ID"`
		ReportID   uint64                 `json:"report_id" gorm:"not null;index:idx_report_id;uniqueIndex:uniq_eval_user_report_deleted;comment:关联的面试报告ID"`
		...
		Deleted    int                    `json:"deleted" gorm:"default:0;index:idx_deleted;uniqueIndex:uniq_eval_user_report_deleted;comment:是否删除"`
...
// UpsertEvaluation 按 user_id + report_id + deleted 幂等写入评估
func (dao *_InterviewEvaluation) UpsertEvaluation(evaluation *InterviewEvaluation) error {
	if getDB == nil {
		panic("getDB function not initialized, please call model.SetDBGetter first")
	}
	return getDB().Clauses(clause.OnConflict{
		...
		DoUpdates: clause.AssignmentColumns([]string{"comment", "score", "dimensions", "updated_at"}),
