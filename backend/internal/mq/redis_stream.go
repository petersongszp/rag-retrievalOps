...
// RedisStreamQueue implements queueing on Redis Stream with consumer groups.
...
func (q *RedisStreamQueue) Publish(ctx context.Context, message *Message) error {
	...
	log.Printf("[RedisStream] Message published to %s with ID: %s", streamKey, result.Val())
	return nil
}
...
func (q *RedisStreamQueue) Subscribe(ctx context.Context, handler MessageHandler) error {
	...
	log.Printf("[RedisStream] Consumer groups initialized")
				...
				for _, h := range handlers {
					...
					err := q.pool.Submit(func() {
						if err := handlerFn(ctx, msgCopy); err != nil {
							log.Printf("[RedisStream] Error processing message: %v, type: %s, ID: %s", err, msgCopy.Type, msgID)
							return
						}
