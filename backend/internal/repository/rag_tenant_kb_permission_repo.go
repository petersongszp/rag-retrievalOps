...
func (r *RAGTenantKBPermissionRepository) ListByKBID(tenantID, kbID uint64) ([]*model.RAGTenantKBPermission, error) {
	var permissions []*model.RAGTenantKBPermission
	err := r.db.Where("tenant_id = ? AND kb_id = ?", tenantID, kbID).Order("created_at DESC").Find(&permissions).Error
	return permissions, err
}

func (r *RAGTenantKBPermissionRepository) Update(permission *model.RAGTenantKBPermission) error {
	return r.db.Model(&model.RAGTenantKBPermission{}).
		Where("tenant_id = ? AND kb_id = ?", permission.TenantID, permission.KBID).
