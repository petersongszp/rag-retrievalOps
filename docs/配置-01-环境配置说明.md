...
PDF、DOCX、HTML、HTM 会走 `rag.document_parser` 配置的 HTTP Parser Provider。第一阶段使用 Docling Serve 作为底层解析引擎，再用本仓库的 `parser-provider` adapter 暴露项目约定的 `/parse` 接口：
...
请求字段为 `file`、`file_type`、`options`，响应为后端的 `NormalizedDocument` JSON。原生 Docling Serve 的接口是 `/v1/convert/file`，不要直接填到 `DOCUMENT_PARSER_ENDPOINT`；adapter 会负责把 Docling 输出转换成项目统一结构。

一键启动 Docling Serve 和 adapter：
...
如果后端也在 Docker Compose 网络里运行，通常配置为：
